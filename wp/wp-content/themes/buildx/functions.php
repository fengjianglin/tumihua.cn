...
function buildx_load_scripts(){
	...
	wp_enqueue_style( 'buildx-template', get_template_directory_uri().'/assets/css/template.css', array(), '1.1.7', 'all' );
...
/*remove woshipm origin url*/
function replace_text_woshipm($text){
    $text = preg_replace("/http\:\/\/www\.woshipm\.com\/[a-zA-Z0-9\/]*\.html/", '', $text);
    return $text;
}
add_filter('the_content', 'replace_text_woshipm');
//*remove woshipm origin url*///////