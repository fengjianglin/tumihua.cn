...
function buildx_readmore( $id ){
	...
	$readmore = '<span class="post-tags clearfix">'.$taglist.'</span>';
	return wp_kses(
		    $readmore,
            array(
                'span' => array(
...
function buildx_load_dynamic_script(){
	...
	$buildx_color_css = '
		...
		.moretag:hover, #sidebar .btn, footer .btn{border-color: '.$buildx_heading_color.'; background: '.$buildx_heading_color.' !important;}
		...
		body p,.post-tags, .post-meta{ color: '.$buildx_text_color.'}
